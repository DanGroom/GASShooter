...
#include "Characters/Components/GSALSMantleComponent.h"
...
FName UGSALSMantleComponent::NAME_IgnoreOnlyPawn(TEXT("IgnoreOnlyPawn"));

UGSALSMantleComponent::UGSALSMantleComponent()
{
	PrimaryComponentTick.bCanEverTick = true;
	PrimaryComponentTick.bStartWithTickEnabled = true;

	MantleTimeline2 = CreateDefaultSubobject<UTimelineComponent>(NAME_MantleTimeline);
}

void UGSALSMantleComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	if (GetOwner())
	{
		...
		if (OwnerCharacter)
		{
			ALSDebugComponent = OwnerCharacter->FindComponentByClass<UGSALSDebugComponent>();
			...
			MantleTimeline2->SetTimelineFinishedFunc(TimelineFinished);
			MantleTimeline2->SetLooping(false);
			MantleTimeline2->SetTimelineLengthMode(TL_TimelineLength);
			MantleTimeline2->AddInterpFloat(MantleTimelineCurve, TimelineUpdated);

			OwnerCharacter->JumpPressedDelegate.AddUniqueDynamic(this, &UGSALSMantleComponent::OnOwnerJumpInput);
			OwnerCharacter->RagdollStateChangedDelegate.AddUniqueDynamic(
				this, &UGSALSMantleComponent::OnOwnerRagdollStateChanged);
		}
	}
}


void UGSALSMantleComponent::TickComponent(float DeltaTime, ELevelTick TickType,
...
void UGSALSMantleComponent::MantleStart(float MantleHeight, const FALSComponentAndTransform& MantleLedgeWS,
                                      EALSMantleType MantleType)
{
	if (OwnerCharacter == nullptr || !IsValid(MantleLedgeWS.Component) || !IsValid(MantleTimeline2))
	{
		return;
	}

	if (MantleType != EALSMantleType::LowMantle && OwnerCharacter->IsA(AGSHeroCharacter::StaticClass()))
	{
		Cast<AGSHeroCharacter>(OwnerCharacter)->ClearHeldObject();
	}
	...
	MantleTimeline2->SetTimelineLength(MaxTime - MantleParams.StartingPosition);
	MantleTimeline2->SetPlayRate(MantleParams.PlayRate);
	MantleTimeline2->PlayFromStart();
...
bool UGSALSMantleComponent::MantleCheck(const FALSMantleTraceSettings& TraceSettings, EDrawDebugTrace::Type DebugType)
{
	if (!OwnerCharacter)
	{
		return false;
...

	{
		...
		if (ALSDebugComponent && ALSDebugComponent->GetShowTraces())
		{
			UGSALSDebugComponent::DrawDebugSphereTraceSingle(World,
			                                               TraceStart,
			                                               TraceEnd,
...
void UGSALSMantleComponent::Server_MantleStart_Implementation(float MantleHeight,
...
void UGSALSMantleComponent::Multicast_MantleStart_Implementation(float MantleHeight,
                                                               const FALSComponentAndTransform& MantleLedgeWS,
                                                               EALSMantleType MantleType)
{
...
void UGSALSMantleComponent::MantleUpdate(float BlendIn)
{
	if (!OwnerCharacter)
	{
		return;
	...
	const FVector CurveVec = MantleParams.PositionCorrectionCurve
	                                     ->GetVectorValue(
		                                     MantleParams.StartingPosition + MantleTimeline2->GetPlaybackPosition());
...
void UGSALSMantleComponent::MantleEnd()
{
	...
	if (OwnerCharacter)
	{
		...
		if (OwnerCharacter->IsA(AGSHeroCharacter::StaticClass()))
		{
			Cast<AGSHeroCharacter>(OwnerCharacter)->UpdateHeldObject();
		}
	}
...
void UGSALSMantleComponent::OnOwnerJumpInput()
{
...
void UGSALSMantleComponent::OnOwnerRagdollStateChanged(bool bRagdollState)
{
	// If owner is going into ragdoll state, stop mantling immediately
	if (bRagdollState)
	{
		MantleTimeline2->Stop();
	}
}
