...
AGSALSPlayerCameraManager::AGSALSPlayerCameraManager()
{
...
void AGSALSPlayerCameraManager::OnPossess(AGSCharacterBase* NewCharacter)
{
	...
	ALSDebugComponent = ControlledCharacter->FindComponentByClass<UGSALSDebugComponent>();
}

float AGSALSPlayerCameraManager::GetCameraBehaviorParam(FName CurveName) const
{
...
void AGSALSPlayerCameraManager::UpdateViewTargetInternal(FTViewTarget& OutVT, float DeltaTime)
{
	...
	if (OutVT.Target)
	{
		...
		if (OutVT.Target->IsA<AGSCharacterBase>())
		{
...
FVector AGSALSPlayerCameraManager::CalculateAxisIndependentLag(FVector CurrentLocation, FVector TargetLocation,
...
bool AGSALSPlayerCameraManager::CustomCameraBehavior(float DeltaTime, FVector& Location, FRotator& Rotation, float& FOV)
{
